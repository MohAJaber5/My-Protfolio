...
const FeatureCard = ({ icon, title, description, index }: FeatureCardProps) => {
  ...
  return (
    ...
    >
      <div className="rounded-full w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center mb-4 sm:mb-5" style={{
        backgroundColor: 'hsl(var(--primary) / 0.2)',
        color: 'hsl(var(--primary))'
      }}>
        {icon}
      </div>
      <h3 className="text-lg sm:text-xl font-semibold mb-2 sm:mb-3" style={{ color: 'hsl(var(--foreground))' }}>{title}</h3>
      <p className="text-sm sm:text-base" style={{ color: 'hsl(var(--muted-foreground))' }}>{description}</p>
    </div>
  );
};
